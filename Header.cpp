...
{
	int number;
	auto find_place = [this](int elem)->ptrNODE
	{
		ptrNODE result = head;
		while (result->next && result->next->info->value < elem)
			result = result->next;
		return result;
	};
	while (!file.eof())
...
void LIST::print_count(std::ofstream& file)
{
	ptrNODE p = head->next;
	if (this->size == 0)
	{
...
void LIST::sorting()
{
	...
	while (h)
	{
		if ((h_prev->info->value) > (h->info->value))
		{
			ptrNODE p = head;
			while (p->next != h && (p->next->info->value) < (h->info->value))
				p = p->next;
